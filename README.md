...
ProofLine is a take-home prototype for a US Treasury / TTB-style alcohol-label verification assessment. It is a single FastAPI + SQLite container with in-container OCR, a deterministic YAML rule engine, Ed25519-signed receipts, an SSE-streamed state machine, and an async batch pipeline — built by a *governed multi-LLM swarm* whose coordination is itself documented and evidence-gated. The pitch in one line: **a governed-AI compliance engine, built by a governed-AI engineering swarm, with signed receipts at both layers.**
...
- **Storage is in-process for this prototype slice.** Runs, receipts, and the result cache live in the single container; the SPEC's SQLite-WAL + local-artifact design and the storage *adapter* are the documented swap to Postgres + object-lock storage (Scaling path). This is ephemeral by design for a single-container demo and is called out as such.
...
- **SQLite-WAL persistence and the S3/Postgres adapters** are designed and seam-ready; the running prototype uses in-process storage for the single-container slice.
